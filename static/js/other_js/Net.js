...
class Net {
  constructor() {
    ...
    //   if ((data = "CONNECTED")) net.loadAlbum("Eminem");
    // });
    this.loadAlbum("Eminem");
  }
  ...
  getPlaylistsToPlay(database) {
    $.ajax({
      ...
      success: function (data) {
        ...
        if (obj.actionBack == "LOADED") {
          ...
          if (playlists.length) {
            playlists.forEach((playlist) => {
              ...
              let smallDiv = $("<div>").addClass("playlistName").html(playlist);
              bigDiv.append(smallDiv);
              smallDiv = $("<div>").addClass("songButtons");
              let container = $("<div>").addClass("buttonsContainer");
              let img = $("<img>")
                ...
                .on("click", function (e) {
                  e.stopPropagation();
                  let agree = confirm(
                    "Czy na pewno chcesz usunąć playlistę " + playlist
                  );
                  if (agree) net.deletePlaylist(database, playlist, bigDiv);
                });
        ...
        } else alert("NIE ZAŁADOWANO PLAYLIST");
      },
      error: function (xhr, status, error) {
        console.log(xhr);
      },
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (obj.actionBack == "LOADED") {
          ...
          $("#songs").empty();
          net.createUserPanel();
          let bigDiv = $("<div>").prop("id", "mainPhrases");
          let smallDiv = $("<div>").prop("id", "playlistNameDiv");
          let input = $("<input>")
          ...
          let bt = $("<button>")
            ...
            .on("click", function () {
              if (!checkIfExist($("#newPlaylistName").val()))
                net.createNewPlaylist(
                  database,
                  $("#newPlaylistName").val(),
                  songInfo
                );
              else alert("JUŻ ISTNIEJE TAKA PLAYLISTA");
              $("#newPlaylistName").val("");
              $("#newPlaylistName").focus();
              function checkIfExist(playlistName) {
                let exist = false;
                playlists.forEach((playlist) => {
                  if (playlistName == playlist) exist = true;
                });
                return exist;
              }
            });
  ...
  createNewPlaylist(database, playlistName, songInfo) {
    $.ajax({
      ...
      type: "POST",
      success: function (data) {
        if (data == "CREATED") net.getPlaylistsToAdd(database, songInfo);
        else alert("NIE UTWORZONO");
      },
      error: function (xhr, status, error) {
        console.log(xhr);
      },
      ...
      success: function (data) {
        ...
        if (obj.actionBack == "ADDED") {
          if (music.currentAlbumPlay == playlistName && music.playlistPlaying) {
            ...
            if (music.random) music.queue.push(music.queue.length);
            else {
              let lastIndex = music.queue.findIndex(
                (e) => e == music.queue.length - 1
              );
              if (lastIndex < music.currentSongIndex) music.currentSongIndex++;
        ...
        } else if (obj.actionBack == "EXIST") alert("JUŻ TAM JEST");
        else alert("NIE DODANO");
      },
      error: function (xhr, status, error) {
        console.log(xhr);
      },
      ...
      success: function (data) {
        if ((data = "DELETED")) {
          ...
          if (music.currentAlbumPlay == playlistName && music.playlistPlaying) {
            ...
            if (songIndex < music.currentSongIndex) music.currentSongIndex--;
            for (let i = music.queue.length - 1; i >= 0; i--) {
              if (music.queue[i] == indexToRemove) music.queue.splice(i, 1);
              else if (music.queue[i] > indexToRemove) music.queue[i]--;
            }
            if (music.currentSongIndex == -1)
              music.currentSongIndex = music.queue.length - 1;
            music.currentSongIndex %= music.queue.length;
            if (music.queue.length == 0) music.clearMusic();
            else if (isCurrentSong) ui.changeSong();
          }
        } else alert("NIE USUNIĘTO");
      },
      error: function (xhr, status, error) {
  ...
  createUserPanel() {
    let bigDiv = $("<div>").prop("id", "loginMenu");
  ...
  createSongsLayout(obj, albumName, enableDelete) {
    ...
    net.createUserPanel();
    let bigDiv = $("<div>").prop("id", "mainPhrases");
    let smallDiv = $("<div>").addClass("songAlbum").text("Album");
    bigDiv.append(smallDiv);
    smallDiv = $("<div>").addClass("songTitle").text("Tytuł");
    bigDiv.append(smallDiv);
    smallDiv = $("<div>").addClass("songSize").text("Rozmiar");
    bigDiv.append(smallDiv);
    smallDiv = $("<div>").addClass("songButtons");
    let container = $("<div>")
    ...
    let img = $("<img>")
      ...
      .on("click", function () {
        if (localStorage.getItem("MP3_LOGIN") == null)
          ...
          net_user.getPlaylistsToPlayUser(
            "users",
            localStorage.getItem("MP3_LOGIN")
          );
      });
    container.append(img);
    smallDiv.append(container);
    ...
    if (obj.files.length) {
      obj.files.forEach((file, index) => {
        ...
        let smallDiv = $("<div>").addClass("songAlbum").text(file.album);
        bigDiv.append(smallDiv);
        smallDiv = $("<div>").addClass("songTitle").text(file.title);
        bigDiv.append(smallDiv);
        smallDiv = $("<div>").addClass("songSize").text(file.size);
        bigDiv.append(smallDiv);
        smallDiv = $("<div>").addClass("songButtons");
        let container = $("<div>").addClass("buttonsContainer");
        let img = $("<img>")
        ...
        img = $("<img>")
          ...
          .on("click", function (e) {
            ...
            if (localStorage.getItem("MP3_LOGIN") == null)
              ...
              net_user.getPlaylistsToAddUser(
                "users",
                localStorage.getItem("MP3_LOGIN"),
                file
              );
          ...
          img = $("<img>")
            ...
            .on("click", function (e) {
              ...
              if (localStorage.getItem("MP3_LOGIN") == null)
                ...
                net_user.deleteFromPlaylistUser(
                  "users",
                  localStorage.getItem("MP3_LOGIN"),
                  albumName,
                  index,
        ...
        bigDiv.append(smallDiv);
        if (
          music.currentAlbumLoaded == music.currentAlbumPlay &&
          music.playlistLoaded == music.playlistPlaying &&
          ...
          $(".playButton", music.currentSongDiv).prop(
            "src",
            "gfx/icons/" + $("#globalPlayPause").prop("alt") + ".png"
          );
          $(".playButton", music.currentSongDiv).prop(
            "alt",
            $("#globalPlayPause").prop("alt")
          );
        }
        $("#songs").append(bigDiv);
      });
